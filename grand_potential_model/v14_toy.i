...
#change the parabolic free energy of Fe and FeO to be square functions, this way all three phases have a common tangent line the x axis. For a two components and three phases system, it has zero degree of freedom, meaning only at one fixed temperature can three phases co-exist.
...
[ICs]
  [./IC_mu_o_metal]
    type = FunctionIC
    variable = mu_o
    #function = 'if(x<100, -12.42, if(x<130, -34.5, 0))'
    #function = 'if(x<100, -4.416e-3, if(x<130, -22.08, 0))'
    function = 'if(x<100, -4.416, if(x<130, -22.08, 0))'
    #function = '0'
  [../]
...
[./constants]
  ...
  prop_values = '1.5    1.5     1.5     13.8  1e8  0.1     0.5          1      3.2     3.2    3.2   10                   10                      0.4e4           0.3e4                  0.5e4             0.6e4      0.07 3.1416 0.0001'
[../]
[./m]
  type = ParsedMaterial
  f_name = mu
...
[./omega_gas]
  ...
  function = '-0.5*mu_o^2/Va^2/k_gas - cgas_eq*mu_o/Va-10'
  derivative_order = 2
[../]
[./rho_metal]
  type = DerivativeParsedMaterial
