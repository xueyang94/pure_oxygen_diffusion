#try to change order parameter L into an 'if' function of the order parameters, and use this single L function for all three Allen-Cahn equations
#found that only converges when the three phase mobilities are the same, so the three interfaces have the same mobility(this is when I write order parameter>0 as the criteria)
#if we have order parameter>1e-4 as the criteria, it actually can converge
#unit of lengh is nm
[Mesh]
  type = GeneratedMesh
  dim = 2
...
[./constants]
  ...
  prop_values = '1.5    1.5     1.5     13.8  1e6   1e-4      0.5       1      3.2     3.2     3.2    10                   10                    0.4e4           0.3e4                  0.5e4                  0.6e4'
[../]
[./m]
  type = ParsedMaterial
  f_name = mu
...
[./L]
  ...
  f_name = L
  args = 'eta_metal eta_oxide eta_gas'
  material_property_names = 'L1 L2 L3 L_bulk'
  #Has to choose eta larger than tolerance, not smaller, because it's multi-phase. Both of the two order parameters are almost zero in the third phase.
  #But only at the interface can the two order parameter larger than zero at the same time.
  #function = 'if (eta1>0 & eta2>0, L12, if(eta1>0 & eta3>0, L13, if(eta2>0 & eta3>0, L23, mobility when only one of the order parameters is larger than zero, should it be zero???)))'
  function = 'if (eta_metal>0.119 & eta_oxide>0.119, L1, if(eta_metal>0.119 & eta_gas>0.119, L3, if(eta_oxide>0.119 & eta_gas>0.119, L2, L_bulk)))'
  #function = 'if (eta_metal>0.9 | eta_oxide > 0.9 | eta_gas > 0.9, L_bulk, if (eta_gas < 1e-4, L1, if (eta_metal < 1e-4, L2, L3)))'
  outputs = exodus
[../]
[./view_L]
  type = ParsedMaterial
