#try to change order parameter L into a function of the order parameters, and use this single L function for all three Allen-Cahn equations
#change mobility L to arctan smooth function, based on https://dsp.stackexchange.com/questions/35211/generating-smoothed-versions-of-square-wave-triangular-etc
#use if function for interfaces instead of using a interpolation function, this way we make sure that the interfacial mobility is what we assign it to be
...
[./constants]
  ...
  prop_names = 'g_metal g_oxide g_gas   Va     D  cmetal_eq coxide_eq cgas_eq k_metal k_oxide k_gas interface_energy_sigma interface_thickness_l bulk_mobility phase_mobility_metal phase_mobility_oxide phase_mobility_gas'
  prop_values = '1.5    1.5     1.5     13.8  1e6   1e-4      0.5       1      3.2     3.2     3.2    10                   10                    1e5           6e4                 2e4                   0.5e4'
[../]
[./m]
  type = ParsedMaterial
  f_name = mu
...
[./part1]
  ...
  f_name = part1
  args = 'eta_metal eta_oxide eta_gas'
  material_property_names = 'L12 L_bulk'
  function = '(L12 + L_bulk)/2 + (L12 - L_bulk)/3.14*atan(sin(2*3.14*(eta_metal-0.25))/0.01)'
[../]
[./part2]
  type = ParsedMaterial
  f_name = part2
  args = 'eta_oxide'
  material_property_names = 'L23 L_bulk'
  function = '(L23 + L_bulk)/2 + (L23 - L_bulk)/3.14*atan(sin(2*3.14*(eta_oxide-0.25))/0.01)'
[../]
[./part3]
  type = ParsedMaterial
  f_name = part3
  args = 'eta_gas'
  material_property_names = 'L13 L_bulk'
  function = '(L13 + L_bulk)/2 + (L13 - L_bulk)/3.14*atan(sin(2*3.14*(eta_gas-0.25))/0.01)'
[../]
...
[./L]
  ...
  f_name = L
  args = 'eta_metal eta_oxide eta_gas'
  material_property_names = 'L_bulk part1 part2 part3'
  #function = 'if (eta_metal>0.9999 | eta_oxide > 0.9999 | eta_gas > 0.9999, L_bulk, part1 + part2 + part3)'
  function = 'if (eta_metal>0.9 | eta_oxide > 0.9 | eta_gas > 0.9, L_bulk, if (eta_gas < 1e-4, part1, if (eta_metal < 1e-4, part2, part3)))'
  outputs = exodus
[../]
[./view_L]
  type = ParsedMaterial
...
#   f_name = eta_sum
#   args = 'eta_metal eta_oxide eta_gas'
#   function = 'eta_metal + eta_oxide + eta_gas'
#   output = csv
...
[./switch_12]
  ...
  f_name = switch_12
  args = 'eta_metal eta_oxide eta_gas'
  function = 'eta_metal^2*eta_oxide^2/(eta_metal^2*eta_oxide^2 + eta_oxide^2*eta_gas^2 + eta_metal^2*eta_gas^2)'
[../]
[./switch_23]
  type = ParsedMaterial
  f_name = switch_23
  args = 'eta_metal eta_oxide eta_gas'
  function = 'eta_oxide^2*eta_gas^2/(eta_metal^2*eta_oxide^2 + eta_oxide^2*eta_gas^2 + eta_metal^2*eta_gas^2)'
[../]
[./switch_13]
  type = ParsedMaterial
  f_name = switch_13
  args = 'eta_metal eta_oxide eta_gas'
  function = 'eta_metal^2*eta_gas^2/(eta_metal^2*eta_oxide^2 + eta_oxide^2*eta_gas^2 + eta_metal^2*eta_gas^2)'
[../]
